...
(() => {
    ...
    CraftGame.Close = function() {
        ...
        $.post("http://esx_guncrafting/CloseGame", JSON.stringify({}));
    };

    CraftGame.Failed = function() {
        if (active) $.post("http://esx_guncrafting/CraftingFailed",'{}');
        active = false;
        CraftGame.Close();
    }

    CraftGame.Success = function() {
        if (active) $.post("http://esx_guncrafting/CraftingSuccess",'{}');
        active = false;
        CraftGame.Close();
    }
